...
Deletion reconciliation never interprets an empty local controller registry as proof that durable resources are absent. It first adopts the exact catalog-recorded replica set through the idempotent physical ensure path, then deletes its Ra members and SQLite files, and only then publishes the tombstone. This makes deletion recoverable after a complete Erlite runtime restart, where dynamic database children are intentionally absent.
