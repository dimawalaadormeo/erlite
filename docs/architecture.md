...
Phase 7 adds a catalog-persisted movement protocol and physical mover. A ready database may carry one fenced movement record containing the source, replacement, operation ID, and an `adding` or `removing` phase. The advertised RF=3 replica set remains unchanged while a caught-up source creates a durable, checksummed SQLite snapshot; the snapshot is transferred and verified on the target before its Ra server is added. The replacement must catch up its Ra machine and SQLite applied index through a quorum barrier before the catalog records replacement readiness. Only then is the source removed and its SQLite file deleted, after which completion atomically substitutes the new member and advances the placement generation. Incomplete movements participate in catalog reconciliation discovery, database controllers reconstruct from the observed three- or four-member Ra configuration, and deletion is refused while movement is in progress. The accepted protocol is recorded in ADR 0001.
...
Phase 13 adds `erlite_release` and the supervised `erlite_observability`
service. Cluster join performs the release comparison before persistent local
identity or catalog membership changes. Compatible protocol ranges must
overlap; command format, snapshot format, and canonical SQLite runtime identity
must match exactly. Observability owns a bounded-key ETS counter table and
derives readiness from required-worker liveness plus a consistent catalog
query. It reports aggregate database resource data without putting tenant or
